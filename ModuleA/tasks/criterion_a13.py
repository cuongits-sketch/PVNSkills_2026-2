...
def task_A13_01(task: Task) -> Result:
    """DNS zone check"""
    command = "rndc zonestatus dmz.pvnskills.org."
    score = 0
    cmd_result = None
    msg = f"{task.host.name} is NOT secondary name server for dmz.pvnskills.org"
    if task.host.name == "ha-prx01":
        msg = f"{task.host.name} is NOT primary name server for dmz.pvnskills.org"
    try:
        cmd_result = run_command(task=task, command=command)
        if task.host.name == "ha-prx01" and "type: primary" in cmd_result.result:
            msg = f"{task.host.name} is primary name server for dmz.pvnskills.org"
            score += 0.2
        if task.host.name == "ha-prx02" and "type: secondary" in cmd_result.result:
            msg = f"{task.host.name} is secondary name server for dmz.pvnskills.org"
            score += 0.2
    except Exception:
        pass
...
def task_A13_02(task: Task) -> Result:
    ...
    command = "dig +recurse +time=2 +tries=1 @127.0.0.1 int.pvnskills.org SOA"
    score = 0
    cmd_result = None
...
def task_A13_03(task: Task) -> Result:
    ...
    hostname = "mail.dmz.pvnskills.org."
    ip_addr = "10.1.20.10"
    results = [
        check_host_record(
            task=task,
    ...
    results.append(
        check_host_record(
            task=task,
            record_type=DNS_RECORD_TYPE.A,
            hostname="ha-prx01.dmz.pvnskills.org.",
    ...
    results.append(
        check_host_record(
            task=task,
            record_type=DNS_RECORD_TYPE.A,
            hostname="ha-prx02.dmz.pvnskills.org.",
    ...
    results.append(
        check_host_record(
            task=task,
            record_type=DNS_RECORD_TYPE.A,
            hostname="web01.dmz.pvnskills.org.",
    ...
    results.append(
        check_host_record(
            task=task,
            record_type=DNS_RECORD_TYPE.A,
            hostname="web02.dmz.pvnskills.org.",
...
def task_A13_04(task: Task) -> Result:
    ...
    results = [
        check_host_record(
            task=task,
            record_type=DNS_RECORD_TYPE.A,
            hostname="prx-vrrp.dmz.pvnskills.org.",
    ...
    return Result(
        ...
        command_run=[item for res in results for item in res["command"]],
        command_output=[item for res in results for item in res["command_output"]],
        score=0.1 if sum([res["score"] for res in results]) == 4 else 0.0,
        max_score=0.1,
    )
...
def task_A13_06(task: Task) -> Result:
    """CNAME record for web check"""
    result = check_dns_record(
        task=task,
        record_type=DNS_RECORD_TYPE.CNAME,
        query="www.dmz.pvnskills.org.",
        expected="prx-vrrp.dmz.pvnskills.org.",
    )
