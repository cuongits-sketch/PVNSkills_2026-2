...
def task_A09_01(task: Task) -> Result:
    ...
    # Send mail over SMTP
    command = "printf 'Subject: WSC2024_FLAG\n\nWSC2024_FLAG'  | curl -s -k --ssl-reqd  --url smtps://localhost --user jamie:Skill06@pvnsc -H 'Subject: WSC2024_FLAG' --mail-from 'jamie.oliver@dmz.pvnskills.org(Jamie Oliver)' --mail-rcpt jamie.oliver@dmz.pvnskills.org --upload-file -"
    ...
    cmd_result = None
    command = r"""IFS=" " read -r -a mail_ids <<< "$(curl -s -k --user jamie:Skill06@pvnsc imaps://localhost/INBOX?SUBJECT%20WSC2024_FLAG 2>&1 | grep -oP '(?<=SEARCH)[ 0-9]+')" && curl -s -k --user jamie:Skill06@pvnsc "imaps://localhost/INBOX;MAILINDEX=${mail_ids[-1]}" 2>&1"""
    commands.append(command)
    try:
        cmd_result = run_command(task=task, command=command)
        command_outputs.append(cmd_result.result)
...
def task_A09_02(task: Task) -> Result:
    """test echo service"""
    score = 0
    command = "printf 'Subject: WSC2024_ECHO_FLAG\nFrom: jamie.oliver@dmz.pvnskills.org\nTo: echo@dmz.pvnskills.org\n\nWSC2024_ECHO_FLAG' | curl -s -k --ssl-reqd  --url smtps://localhost --user jamie:Skill06@pvnsc --mail-from 'jamie.oliver@dmz.pvnskills.org(Jamie Oliver)' --mail-rcpt echo@dmz.pvnskills.org --upload-file -"
    commands = [command]
    command_outputs = []
    ...
    cmd_result = None
    command = r"""IFS=" " read -r -a mail_ids <<< "$(curl -s -k --user jamie:Skill06@pvnsc imaps://localhost/INBOX?FROM%20echo@dmz.pvnskills.org 2>&1 | grep -oP '(?<=SEARCH)[ 0-9]+')" && curl -s -k --user jamie:Skill06@pvnsc "imaps://localhost/INBOX;MAILINDEX=${mail_ids[-1]}" 2>&1"""
    commands.append(command)
    try:
        cmd_result = run_command(task=task, command=command)
        command_outputs.append(cmd_result.result)
        ...
        if (
            "to: jamie.oliver@dmz.pvnskills.org"
            and "from: echo@dmz.pvnskills.org"
