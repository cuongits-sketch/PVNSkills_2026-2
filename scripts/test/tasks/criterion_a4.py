...
def task_A04_01(task: Task) -> Result:
    ...
    results = [
        check_host_record(
            ...
            hostname="int-srv01.int.pvnskills.org.",
...
def task_A04_02(task: Task) -> Result:
    """SRV int-srv01 check"""
    command = (
        "dig +short +time=2 +tries=1 @127.0.0.1 _ldap._tcp.auth.int.pvnskills.org SRV"
    )
    score = 0
    msg = "SRV record for int-srv01.int.pvnskills.org does not exist"
    cmd_result = None
    try:
        cmd_result = run_command(task=task, command=command)
        if "10 50 389 int-srv01.int.pvnskills.org" in cmd_result.result:
            msg = "SRV record for int-srv01.int.pvnskills.org exists"
            score = 0.2
    except Exception:
        pass
...
def task_A04_04(task: Task) -> Result:
    ...
    dig_command = "dig +short +time=2 +tries=1 @127.0.0.1 dmz.pvnskills.org SOA"
    score = 0
    msg = "int-srv01 is not a secondary for dmz.pvnskills.org."
    ...
    try:
        ...
        if (
            ...
            msg = "int-srv01 is secondary for dmz.pvnskills.org."
            score += 0.05
    except Exception:
    ...
    try:
        ...
        command_outputs.append(cmd_result.result)
        if "dmz.pvnskills.org." in cmd_result.result:
            score += 0.05
        else:
            msg += " Cannot resolve dmz.pvnskills.org on int-srv01"
    except Exception:
        command_outputs.append(UNKNOWN_MSG)
