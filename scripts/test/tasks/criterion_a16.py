...
def task_A16_01(task: Task) -> Result:
    ...
    command = "dig +short +time=2 +tries=1 @1.1.1.10 dmz.pvnskills.org SOA"
    second_command = (
        "dig +tcp +short +time=2 +tries=1 @1.1.1.10 dmz.pvnskills.org SOA"
    )
    score = 0
...
def task_A16_05(task: Task) -> Result:
    """End-to-End test"""
    command = "dig +time=2 +tries=1 +short @10.1.10.10 int.pvnskills.org SOA"
    score = 0
    commands = [command]
    command_outputs = []
    ...
    try:
        cmd_result = run_command(task=task, command=command)
        command_outputs.append(cmd_result.result)
        if "pvnskills.org" in cmd_result.result:
    ...
    command = (
        "dig +time=2 +tries=1 +short @2001:db8:1001:10::10 int.pvnskills.org SOA"
    )
    commands.append(command)
    try:
        cmd_result = run_command(task=task, command=command)
