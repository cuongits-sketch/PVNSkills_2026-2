...
def task_A01_01(task: Task) -> Result:
    ...
    try:
        ...
        got_mark = (
            True
            if "dn: ou=Employees,dc=int,dc=pvnskills,dc=org" in cmd_result.result
            else False
        )
    except Exception:
        got_mark = False
...
def task_A01_03(task: Task) -> Result:
    ...
    results = [
        check_ldap_user_attributes(
            task=task, username="jamie", mail="jamie.oliver@dmz.pvnskills.org"
        )
    ]
    results.append(
        check_ldap_user_attributes(
            task=task, username="peter", mail="peter.fox@dmz.pvnskills.org"
        )
    )
