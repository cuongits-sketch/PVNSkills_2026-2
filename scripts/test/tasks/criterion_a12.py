...
from scripts.test.tasks.common.helper import UNKNOWN_MSG, task_get_ca_cert
...
def task_A12_03(task: Task) -> Result:
    ...
    command = "curl -ks --connect-timeout 2 https://www.dmz.pvnskills.org/whoami 2>&1"
    score = 0
    commands = []
    command_outputs = []
...
def task_A12_04(task: Task) -> Result:
    ...
    command = (
        "curl -kvs --connect-timeout 2 https://www.dmz.pvnskills.org/whoami 2>&1"
    )
    score = 0
    cmd_result = None
...
def task_A12_05(task: Task) -> Result:
    ...
    command = "curl -kvs -I --connect-timeout 2 http://www.dmz.pvnskills.org/ 2>&1"
    score = 0
    cmd_result = None
...
def task_A12_06(
    ...
    command = """timeout 2 bash -c 'echo "Q" | openssl s_client -connect www.dmz.pvnskills.org:443 -CAfile /tmp/ca.pem || echo "Q" | openssl s_client -connect www.dmz.pvnskills.org:443' 2>&1 || exit 0"""
    score = 0
    cmd_result = None
...
def task_A12_07(task: Task) -> Result:
    ...
    command = "curl -4vks --connect-timeout 2 https://www.dmz.pvnskills.org 2>&1 ; curl -6vks --connect-timeout 2 https://www.dmz.pvnskills.org 2>&1"
    score = 0
    cmd_result = None
